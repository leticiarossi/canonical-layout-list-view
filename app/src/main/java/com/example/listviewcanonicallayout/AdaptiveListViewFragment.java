...
public class AdaptiveListViewFragment extends Fragment {
    ...
    private final EmailAdapter.EmailAdapterListener emailAdapterListener =
            new EmailAdapter.EmailAdapterListener() {
                @Override
                public void onEmailClicked(@NonNull View view, long emailId) {
                    AdaptiveListViewDetailFragment fragment =
                            AdaptiveListViewDetailFragment.newInstance(emailId);
                    ...
                    if (detailViewContainerId == R.id.list_view_detail_fragment_container) {
                        getParentFragmentManager()
                                ...
                                .replace(detailViewContainerId, fragment, AdaptiveListViewDetailFragment.TAG)
                                .commit();
                    } else {
                        getParentFragmentManager()
                                .beginTransaction()
                                .addSharedElement(view, ViewCompat.getTransitionName(view))
                                .replace(detailViewContainerId, fragment, AdaptiveListViewDetailFragment.TAG)
                                .addToBackStack(AdaptiveListViewDetailFragment.TAG)
                                .commit();
                    }
                }
            };
    ...
    private void setEmailSelected(long emailId, boolean selected) {
        ...
        EmailAdapter.EmailViewHolder emailViewHolder =
                (EmailAdapter.EmailViewHolder) emailList.findViewHolderForItemId(emailId);
        if (emailViewHolder != null) {
            ((MaterialCardView) emailViewHolder.container).setChecked(selected);
        }
        EmailData.Email email = EmailData.getEmailById(emailId);
    ...
    static final class EmailAdapter extends RecyclerView.Adapter<EmailAdapter.EmailViewHolder> {
        ...
        private void updateEmailSelected(@NonNull EmailViewHolder holder) {
            ...
            if (holder.container.getResources().getConfiguration().orientation
                ...
                EmailData.Email email = EmailData.getEmailById(holder.getItemId());
        ...
        public void onBindViewHolder(@NonNull EmailViewHolder holder, int position) {
            ...
            holder.senderTitle.setText(resources.getString(R.string.sender_title));
            holder.emailTitle.setText(resources.getString(R.string.email_title));
            holder.emailTitle.append(" " + (emailId + 1));
            holder.emailPreview.setText(resources.getString(R.string.email_text));
